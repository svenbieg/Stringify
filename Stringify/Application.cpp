...
while(!task->Cancelled)
	...
	auto dispatched=Dispatch(result_box, &TextBox::AppendLine, line);
	if(!dispatched)
		break;
	dispatched->Wait();
...
{
HICON hicon=(HICON)LoadImage(NULL, path->Begin(), IMAGE_ICON, 0, 0, LR_LOADFROMFILE|LR_LOADTRANSPARENT);
