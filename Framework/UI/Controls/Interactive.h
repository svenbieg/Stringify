...
public:
	...
	Event<Interactive, Interactive*, FocusReason> Focused;
	Event<Interactive, Interactive*, FocusReason> FocusLost;
	virtual Handle<Cursor> GetCursor();
	BOOL HasFocus();
	BOOL HasPointerFocus();
	static Interactive* GetNextControl(Window* Window, Interactive* Control, BOOL Forward=true);
	BOOL IsCapturingPointer();
	BOOL IsEnabled();
	Event<Interactive, Handle<KeyEventArgs>> KeyDown;
	Event<Interactive, Handle<KeyEventArgs>> KeyPressed;
	Event<Interactive, Handle<KeyEventArgs>> KeyUp;
	BOOL KillFocus(FocusReason Reason);
	...
	static VOID SetPointerFocus(Interactive* Focus);
	BOOL TabStop;
...
private:
	...
	// Common
	static BOOL GetNext(Window* Window, Interactive* Control, Interactive** Next, BOOL Forward, UINT Level);
	...
	VirtualKey m_KeyDown;
	static Interactive* s_PointerFocus;
};
